...
void SystemInit (void)
{
    ...
    // HSITRIM default to 16, each point aprox. 40KHz
    //todo a 0 baja freq (mas chico menos freq)
    // RCC->CR &= ~RCC_CR_HSITRIM;
    // RCC->CR |= (16 << 3);    // default to 16
    ...
    // Flash wait states; 0 sysclk < 24MHz; 1 sysclk < 48MHz; 2 sysclk < 72MHz
    // 2 wait state for sysclk 64MHz;
    FLASH->ACR &= (uint32_t)((uint32_t)~FLASH_ACR_LATENCY);
    FLASH->ACR |= (uint32_t)FLASH_ACR_LATENCY_2;
    ...
    // APB2 prescaler HCLK -> PCLK2
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV1;
    
    // APB1 prescaler HCLK -> PCLK1
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE1_DIV2;
